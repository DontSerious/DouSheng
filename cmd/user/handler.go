...
import (
	...
	"github.com/DontSerious/DouSheng/cmd/user/service"
...
func (s *UserServiceImpl) Follow(ctx context.Context, req *user.FollowOperationRequest) (resp *user.FollowOperationResponse, err error) {
	...
	default:
		resp.BaseResp = pack.BuildBaseResponse(errno.ServiceErrCode, errno.ServiceErr.ErrMsg)
	}
	return resp, nil
}
...
func (s *UserServiceImpl) Authentication(ctx context.Context, req *user.AuthenticationRequest) (resp *user.AuthenticationResponse, err error) {
	...
	if len(req.Username) == 0 {
		resp.BaseResp = pack.BuildBaseResponse(errno.TokenInvalidErrCode, errno.TokenErr.ErrMsg)
		resp.UserId = 0
		return resp, nil
	}
